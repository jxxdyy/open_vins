...
void TrackApril::feed_new_camera(const CameraData &message) {
  ...
  // There is not such thing as stereo tracking for april
...
void TrackApril::perform_tracking(double timestamp, const cv::Mat &imgin, size_t cam_id, const cv::Mat &maskin) {
  ...
  //* Perform extraction
  image_u8_t img_april = { 
        .width = img0.cols,
        .height = img0.rows,
        .stride = img0.cols,
        .buf = img0.data
        };
  zarray_t *detections = apriltag_detector_detect(tag_detector, &img_april);

  for (int i = 0; i < zarray_size(detections); i++) {
    apriltag_detection_t *detection;
    zarray_get(detections, i, &detection);
    
    ids_april[cam_id].push_back(detection->id);
    centers[cam_id].push_back(cv::Point2f(detection->c[0], detection->c[1]));
    corners[cam_id].push_back(std::vector<cv::Point2f>(4));
    corners[cam_id][i][0] = cv::Point2f(detection->p[0][0], detection->p[0][1]);
    corners[cam_id][i][1] = cv::Point2f(detection->p[1][0], detection->p[1][1]);
    corners[cam_id][i][2] = cv::Point2f(detection->p[2][0], detection->p[2][1]);
    corners[cam_id][i][3] = cv::Point2f(detection->p[3][0], detection->p[3][1]);
  }
  ...
  if (do_downsizing) {
    for (size_t i = 0; i < corners[cam_id].size(); i++) {
      ...
      centers[cam_id].at(i).x *= 2;
      centers[cam_id].at(i).y *= 2;
    }
...
void TrackApril::display_active(cv::Mat &img_out, int r1, int g1, int b1, int r2, int g2, int b2, std::string overlay) {
  ...
  auto ids_april_cache = ids_april;
  auto corners_cache = corners;
  auto centers_cache = centers;
  ...
  for (auto const &pair : img_last_cache) 
  {
    ...
    //! draw april
    if (!ids_april_cache[pair.first].empty()) 
    {
      size_t corners_size = corners_cache[pair.first].size();
      for (int i = 0; i < corners_size; i++) {
        const auto &corner = corners_cache[pair.first][i];
        const auto &center = centers_cache[pair.first][i];
        const auto &tag_id = ids_april_cache[pair.first][i];
        line(img_temp, cv::Point(corner[0].x, corner[0].y),
                       cv::Point(corner[1].x, corner[1].y),
                     cv::Scalar(0xff, 0xff, 0), 2);
        line(img_temp, cv::Point(corner[0].x, corner[0].y),
                       cv::Point(corner[3].x, corner[3].y),
                     cv::Scalar(0xff, 0xff, 0), 2);
        line(img_temp, cv::Point(corner[1].x, corner[1].y),
                       cv::Point(corner[2].x, corner[2].y),
                     cv::Scalar(0xff, 0xff, 0), 2);
        line(img_temp, cv::Point(corner[2].x, corner[2].y),
                       cv::Point(corner[3].x, corner[3].y),
                     cv::Scalar(0xff, 0xff, 0), 2);
        ...
        cv::Size textsize = cv::getTextSize(text, fontface, fontscale, 2,
                                          &baseline);
        cv::putText(img_temp, text, cv::Point(center.x-textsize.width/2, center.y+textsize.height/2),
                  fontface, fontscale, cv::Scalar(0, 0, 0xff), 3);
      }
    }
